...
export async function loadHand(hand: HandStyle): Promise<boolean> {
  ...
  if (!descriptor.googleParam) {
    return document.fonts.check(`1em '${descriptor.family}'`)
  }
  ...
  try {
    const faces = await document.fonts.load(`1em '${descriptor.family}'`)
    ...
    // an error.
    if (faces.length === 0 && !document.fonts.check(`1em '${descriptor.family}'`)) return false
...
export function isHandReady(hand: HandStyle): boolean {
  ...
  const available = document.fonts.check(`1em '${descriptor.family}'`)
